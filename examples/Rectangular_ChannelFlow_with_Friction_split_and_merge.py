...
class RectangularOpenChannelFlow(Application):
    ...
    def create_equations(self):
        equations = [
            ...
            Group(
                equations=[
                   FindMergeable(dest='fluid', sources=['fluid'], A_min=A_min,
                                 x_min=425, x_max=800, y_min=0,
                                 y_max=y_max-dx)
                   ],update_nnps=True
                       ),
            Group(
                equations=[
                   InitialDensityEvalAfterMerge(dest='fluid', sources=['fluid'])
                   ]
                       ),
            Group(
                equations=[
            ...
            Group(
                equations=[
                    CheckForParticlesToSplit(
                        dest='fluid', h_max=h_max, A_max=A_max, x_min=0,
                        x_max=0.75*dx, y_min=dx, y_max=y_max-dx)
                    ],
                ),
            ]
        return equations
